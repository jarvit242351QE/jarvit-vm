...
interface Manifest {
  version: string;
  files: Record<string, string | FileInfo>;
}
...
function normalize(entry: string | FileInfo, filePath: string): FileInfo {
  if (typeof entry === "string") {
    let type: FileInfo["type"] = "feature";
    if (filePath.startsWith("/opt/jarvit/scripts/")) type = "system";
    else if (filePath === "/sbin/init" || filePath === "/opt/jarvit/entrypoint.sh") type = "system";
...
function findSrc(updateDir: string, absPath: string): string | null {
...
function applyUpdate(
  ...
  version: string,
  log: (msg: string) => void
): { ok: boolean; updated: number; skipped: number; kept: number } {
  ...
  const newManifest: Manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8"));
  let currentManifest: Manifest = { version: "unknown", files: {} };
  try {
...
function startChecker(log: (msg: string) => void): void {
  const check = () => {
    try {
      ...
      if (result.ok) {
        ...
        try { fs.rmSync(updatePath, { recursive: true, force: true }); } catch {}
      } else {
