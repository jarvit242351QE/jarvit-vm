...
async function applyUpdate(req: UpdateRequest): Promise<UpdateResult> {
  ...
  console.log(
    `[vm-updater] Applying update ${req.previous} -> ${req.version}`
  );
  console.log(
    `[vm-updater] Files in update: ${Object.keys(newManifest.files).length}`
  );
  ...
  for (const filePath of Object.keys(currentManifest.files)) {
    if (!(filePath in newManifest.files)) {
      ...
      if (currentChecksum === oldChecksum) {
        ...
        console.log(
          `[vm-updater] KEPT (user modified, removed in update): ${filePath}`
        );
        result.conflicts.push({
          path: filePath,
          resolution: "kept_user",
  ...
  // 6. Log summary
  console.log(
    `[vm-updater] Update complete: ${result.filesUpdated} updated, ` +
  ...
  if (result.errors.length > 0) {
    ...
    console.log(
      `[vm-updater] WARNING: ${result.errors.length} errors during update`
    );
...
async function handleRequest(
  ...
  try {
    ...
    console.log(
      `[vm-updater] Received update request: ${updateReq.previous} -> ${updateReq.version}`
    );
...
export function afterServerStart(context: any): void {
  const server = context?.server || context?.gateway?.server;

  if (!server) {
    console.log(
      "[vm-updater] No server in context — /system/update endpoint not registered"
    );
